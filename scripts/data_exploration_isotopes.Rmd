...
soil_iso_data4 <- soil_iso_data4 %>% 
  subset(!is.na(pct_OM)) 
# Substring to get lake pairs
soil_iso_data4 <- soil_iso_data4 %>%
  mutate(lake_pair = substring(lake, 1, 2))
# Substring to get basin
soil_iso_data4 <- soil_iso_data4 %>%
  mutate(basin = substring(lake, 1, 1))
```
...
```{r}
soil_iso_data4 %>% 
  ggplot(aes(x = lake_pair, y = n_g_m2, color = fish_status)) +
  geom_boxplot() +
